...
// TestDedupPurge_NeitherFlag verifies that omitting both --batch and
// --all-hidden produces an error mentioning both flag names.
func TestDedupPurge_NeitherFlag(t *testing.T) {
	var batch []string
	var allHidden bool
	cmd := &cobra.Command{Use: "purge-test", SilenceErrors: true}
	sub := &cobra.Command{
		Use: "dedup-purge",
	...
	cmd.AddCommand(sub)
	cmd.SetArgs([]string{"dedup-purge"})
...
// TestDedupPurge_MutualExclusion verifies that passing both --batch and
// --all-hidden is rejected by cobra.
func TestDedupPurge_MutualExclusion(t *testing.T) {
	var batch []string
	var allHidden bool
	cmd := &cobra.Command{Use: "purge-test", SilenceErrors: true}
	sub := &cobra.Command{Use: "dedup-purge", RunE: func(cmd *cobra.Command, args []string) error { return nil }}
	sub.Flags().StringArrayVar(&batch, "batch", nil, "")
	sub.Flags().BoolVar(&allHidden, "all-hidden", false, "")
	sub.MarkFlagsMutuallyExclusive("batch", "all-hidden")
	cmd.AddCommand(sub)
	cmd.SetArgs([]string{"dedup-purge", "--batch", "some-id", "--all-hidden"})
